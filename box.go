...
func New(options Options) (*Box, error) {
	...
	var lastErr error      // Edgegate
	var lastErrDesc string // Edgegate
	for i, outboundOptions := range options.Outbounds {
		...
		out, err = outbound.New(
			...
			outboundOptions)
		if err != nil {
			lastErrDesc = fmt.Sprintf("parse outbound[%d] error: %+v", i, err) // Edgegate
			fmt.Println(lastErrDesc)
			lastErr = err // Edgegate
	...
	if len(outbounds) == 0 && lastErr != nil { // Edgegate
		return nil, E.Cause(lastErr, lastErrDesc) // Edgegate
	} // Edgegate

	// Edgegate initalization of router moved to preStart
...
func (s *Box) Close() error {
	...
	for i, out := range s.Router().SortedOutboundsByDependenciesCore() {
