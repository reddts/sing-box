...
type Router struct {
	sortedOutboundsByDependenciesCore []adapter.Outbound // Edgegate
	staticDns                            *StaticDNS         // Edgegate
	ctx                                  context.Context
...
func NewRouter(
	...
	router.staticDns = NewStaticDNS(router, dnsOptions.StaticIPs) // Edgegate
...
func (r *Router) UpdateInterfaces() error {
	r.logger.Info("Edgegate!UpdateInterfaces ")
...
func (r *Router) notifyNetworkUpdate(event int) {
	r.logger.Info("Edgegate!notifyNetworkUpdate ", event)
...
func (r *Router) ResetNetwork() error {
	r.logger.Info("Edgegate!Reseting Network")
	conntrack.Close()

	for _, outbound := range r.sortedOutboundsByDependenciesCore {
...
func (r *Router) SortedOutboundsByDependenciesCore() []adapter.Outbound { // Edgegate
	return r.sortedOutboundsByDependenciesCore
}
...
func (r *Router) doSortOutboundsByDependencies() {
	...
	appendOutbounds = func(out adapter.Outbound) {
		...
		r.sortedOutboundsByDependenciesCore = append(r.sortedOutboundsByDependenciesCore, out)
