...
func NewWireGuard(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.WireGuardOutboundOptions) (*WireGuard, error) {
	...
	outbound := &WireGuard{
		...
		hforwarder:   hforwarder, // Edgegate
	}
...
func (w *WireGuard) Close() error {
	if w.hforwarder != nil { // Edgegate
		w.hforwarder.Close() // Edgegate
	} // Edgegate
	if w.device != nil {
		w.device.Close()
	}
...
func (w *WireGuard) InterfaceUpdated() {
	w.logger.Info("Edgegate! Wirguard! Interface updated!XXX")
	// <-time.After(10 * time.Millisecond)
	// if true {
	// 	return
	// }
	if w.pauseManager.IsNetworkPaused() {
		w.logger.Error("Edgegate! Network is paused!... returning")
		return
	}
	<-time.After(50 * time.Millisecond)
	err := w.device.BindUpdate()
	<-time.After(50 * time.Millisecond)
	// err := fmt.Errorf("Edgegate! downing wireguard interface failed")

	if err != nil {
		w.logger.Error("Edgegate! bind update failed", err)
	}
	// w.logger.Error("Edgegate! downing...")
	// e1 := w.device.Down()
	// if e1 != nil {
	// 	w.logger.Error("Edgegate! downing wireguard interface failed", e1)
	// } else {
	// 	w.logger.Warn("Edgegate! downing   Ok!")
	// }
	// for i := 0; i < 5; i++ {
	// 	if !w.pauseManager.IsNetworkPaused() {
	// 		break
	...
	// w.logger.Warn("Edgegate! uping.... wireguard interface")
	// e2 := w.device.Up()
	// if e2 != nil {
	// 	w.logger.Error("Edgegate! Uping wireguard interface failed", e2)
	// } else {
	// 	w.logger.Warn("Edgegate! OK!Updating wireguard interface")

	// }
	// } else {
	// 	w.logger.Warn("Edgegate! OK2!Updating wireguard interface")
	// }
	return
}

func (w *WireGuard) onPauseUpdated(event int) {
	w.logger.Info("Edgegate! Wirguard! on Pause updated! event=", event)
	...
	case pause.EventNetworkPause: // Edgegate already handled in Interface Updated
		err := w.device.Down()
		w.logger.Info("Edgegate! Wirguard! downing net! err=", err)
		<-time.After(50 * time.Millisecond)
	case pause.EventDeviceWake:
		w.device.Up()
	case pause.EventNetworkWake: // Edgegate already handled in Interface Updated
		err := w.device.Up()
		w.logger.Info("Edgegate! Wirguard! Uping net! err=", err)
		<-time.After(50 * time.Millisecond)
	}
}
